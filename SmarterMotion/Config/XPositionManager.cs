...
namespace SmarterMotion
{
    ...
    public sealed class XPositionManager : XObject
    {
        ...
        public int BindPositionTableByTaskId(int setTaskId)
        {
            ...
            if (!Directory.Exists(dir_positionXml))
            {
                Directory.CreateDirectory(dir_positionXml);
                if (!File.Exists(PositionXml_Path))
                {
                    using (new FileStream(PositionXml_Path, FileMode.Create, FileAccess.Write))
                    {

                    }
                }
                document.AppendChild(document.CreateXmlDeclaration("1.0", "UTF-8", null));
                XmlElement newChild = document.CreateElement(root_positionXml);
                newChild.InnerText = "";
                document.AppendChild(newChild);
                document.Save(PositionXml_Path);
            }
