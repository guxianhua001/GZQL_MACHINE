...
namespace SmarterMotion
{
    ...
    public sealed class XDevice : XObject
    {
        ...
        public void BindDo(int setCardId, int setDoId, int channel, int actDoId, string name, string cardname, bool isInterference = false, int InterferenceThings = -1, double pos = 0, DOSTSTYPE status = DOSTSTYPE.HIGH)
        {
            if (cardMap.ContainsKey(setCardId) == true)
            {
