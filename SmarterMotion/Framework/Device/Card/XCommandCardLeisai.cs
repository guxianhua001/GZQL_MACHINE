...
*
* 类 名： XCommandCardLeisai
* 功 能： N/A
* 唯一标识：5f2c5308-9610-45bb-9d64-e02939d0f9f3
* 
* 变更日期：2023/8/22 22:48:44
* 作者：szb
* 公司：CYG
*----------------------------------------------------------------*/
    ...
    public class XCommandCardLeisai : XCommandCard
    {
        ...
        public override int Stop(int actCardId, int axisId)
        {
            lock (obj)
            {
                try
                {
                    var res = LTDMC.dmc_stop((ushort)actCardId, (ushort)axisId, 0);//减速停止
                    return res;
                }
                catch (Exception)
                {
        ...
        public override int EStop(int actCardId, int axisId)
        {
            lock (obj)
            {
                try
                {
                    var res = LTDMC.dmc_stop((ushort)actCardId, (ushort)axisId, 1);//紧急停止
                    return res;
                }
                catch (Exception)
                {
