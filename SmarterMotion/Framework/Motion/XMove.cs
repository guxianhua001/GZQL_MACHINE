...
namespace SmarterMotion
{
    public class XMove : XCtrlEventHandler
    {
        ...
        private int m_actCardId;
        private int m_CoordId;
            ...
            if (m_Move == true)
            {
                ...
                for (int i = 0; i < m_Count; i++)
                {
                    if (m_MoveMode == MoveMode.MOVE_HOME)
                    {
                        ...
                        //------------------雷塞专用，对于其他卡有待验证----------------------------------------------
                        if (XDevice.Instance.FindAxisById(m_AxisId[i]).IsHomeOk == false)
                        {
                            bDone = false;
                        }
                    }
                    else
                    {
                        ...
                        ushort cardid = (ushort)(XDevice.Instance.FindAxisById(m_AxisId[i]).CardId - 1);
                        ushort axisid = (ushort)(XDevice.Instance.FindAxisById(m_AxisId[i]).ActId);
                        //(XDevice.Instance.FindAxisById(m_AxisId[i]).IsMDN == false)  //=true是停止运动
                        if (m_MoveMode == MoveMode.MOVE_LINEABS)
                        {
                            if (LTDMC.dmc_check_done_multicoor(cardid, (ushort)m_CoordId) == 0)
                            {
                                bDone = false;// == 0运动中
                            }
                        }
                        else
                        {
                            ...
                            {
                                bDone = false;//LTDMC.dmc_check_done(_CardID, axis) == 0运动中
                            }
                        }
                        if (XDevice.Instance.FindAxisById(m_AxisId[i]).IsFeedback)
                        {
                            if (m_CheckLmt)
                            {
                                double posn = 0;// XDevice.Instance.FindAxisById(m_AxisId[i]).POS;
                                LTDMC.dmc_get_position_unit(cardid, axisid, ref posn);
                                var dev = Math.Abs(posn - m_Pos[i]);
                                if (dev > 0.05)
                                {
                                    bDone = false;
                                }
                    ...
                    if (m_MoveMode == MoveMode.MOVE_HOME)
                    {
                        ...
                        for (int i = 0; i < m_Count; i++)
                        {
                            if (XDevice.Instance.FindAxisById(m_AxisId[i]).IsALM != true && XDevice.Instance.FindAxisById(m_AxisId[i]).IsASTP != true
                                && XDevice.Instance.FindAxisById(m_AxisId[i]).IsHomeOk == true)
                            {
        ...
        private int PrimOnMoveHome()
        {
            ...
            for (int i = 0; i < m_Count; i++)
            {
                bool eStop = XDevice.Instance.FindAxisById(m_AxisId[i]).HasEStoped;
                bool svonOff = XDevice.Instance.FindAxisById(m_AxisId[i]).HasSVONOFF;
                if (eStop == true || svonOff)//eStop == true|| svonOff
                {
                    XDevice.Instance.FindAxisById(m_AxisId[i]).SetHome(false);
        ...
        private int PrimOnMoveLineAbs()
        {
            ...
            XDevice.Instance.FindCardById(m_actCardId).MoveLineAbs(m_actCardId,m_CoordId, m_AxisId, m_Pos, m_Speed);
            m_Move = true;
            m_MoveMode = MoveMode.MOVE_LINEABS;
            return 0;
        }
        ...
        private int PrimOnPause()
        {
            ...
            if (m_MoveMode != MoveMode.APS_PT_START)
            {
                for (int i = 0; i < m_Count; i++)
                {
                    XDevice.Instance.FindAxisById(m_AxisId[i]).Stop();//XDevice.Instance.FindAxisById(m_AxisId[i]).EStop();
                }
                m_Move = false;
            }
            m_Status = 1;
        ...
        private int PrimOnContinue()
        {
            if (m_Pause)
            {
                if (m_AllowContinue == false)//继续前是否在运动
                {
                    PrimOnDone();
                    return 0;
                }
                ...
                else if (m_MoveMode == MoveMode.MOVE_LINEABS)
                {
                    XDevice.Instance.FindCardById(m_actCardId).MoveLineAbs(m_actCardId, m_CoordId, m_AxisId, m_Pos, m_Speed);
                }
                m_Move = true;
        ...
        private int PrimOnStop()
        {
            for (int i = 0; i < m_Count; i++)
            {
                XDevice.Instance.FindAxisById(m_AxisId[i]).Stop();
            }
            m_Stop = true;
            m_Status = -1;
            PrimOnExit();
        ...
        public int MoveLineAbs(int actCardId,int coordId, int[] axisId, double[] pos, double vel, bool checkLmt = true)
        {
            m_actCardId = actCardId;
            int count = axisId.Length;
            m_Speed = vel;
            m_CoordId = coordId;
            for (int i = 0; i < count; i++)
            {
        ...
        public int MoveEStop()
        {
            PostEvent(this, XEventID.ESTOP);
            return 0;
        }
