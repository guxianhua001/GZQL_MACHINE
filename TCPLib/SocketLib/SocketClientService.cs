 ...
 * 命名空间：TCPLib.Socket
 * 唯一标识：77a3a993-8547-4197-81b1-55752da742ee
 * 文件名：SocketClientService
 * 
 * 创建者：szb
 * 创建时间：2024/8/12 15:03:24
 * 版本：V1.0.0
 * 描述：
 *
 * ----------------------------------------------------------------
    ...
    public class SocketClientService
    {
        ...
        public async void SendMsg(string msg)
        {
            ...
            if (m_tcpClient != null && m_tcpClient.Connected)
            {
                try
                {
                    await m_tcpClient.GetStream().WriteAsync(byteData, 0, byteData.Length);
        ...
        private async Task ReceMsg()
        {
            if (m_tcpClient != null && m_tcpClient.Connected)
            {
                ...
                try
                {
                    ReceLen = await m_tcpClient.GetStream().ReadAsync(m_buffRece, 0, m_buffRece.Length);
