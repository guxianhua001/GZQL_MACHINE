...
需求：启用 Z向校准时，根据所选针头（needleIndex 0→Dz₂，1→Dz₃），将该 Z 轴与 Dx、Dy 一起纳入连续插补（3轴连续插补），使针头在走胶过程中跟随 CAD 表面 Z 轮廓。点胶高度计算：以段内第 1 个点的 Z 为基准 0，每点 Z 目标 = 基准高度 + deltaZ\_i，其中 deltaZ\_i = `CadPoint.Z_i - CadPoint.Z_1`（Z 轴越往下数值越大）。
