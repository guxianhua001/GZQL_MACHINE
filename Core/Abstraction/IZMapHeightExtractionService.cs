...
{
    /// <summary>
    /// ZMAP高度图Z值提取服务——核心作用是把"DXF/CAD轨迹点"与"ZMAP高度图"这两个互不相关的
    /// 坐标系,通过共同的机械坐标系间接对齐，从而按轨迹点的机械XY坐标反查ZMAP图像像素位置，
    /// 双线性采样得到该处高度，最终提取出逐点Z值供Step3面板写回 CadPoint.Z。
    ///
    /// 对齐链路：
    ///   CAD(X,Y) --Step4已有N点仿射--> 机械坐标(MachineX,MachineY)
    ///   机械坐标(MachineX,MachineY) --本服务标定的仿射逆变换--> ZMAP像素(PixelCol,PixelRow)
    ///   ZMAP像素(PixelCol,PixelRow) --双线性采样--> 原始高度RawZ --+ZOffset基准修正--> CorrectedZ
    ///
