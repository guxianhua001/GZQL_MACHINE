...
namespace HalconWrapper
{
    ...
    public partial class VMHWindowControl : UserControl
    {
        ...
        public double positionX,
            positionY;
        private bool isMoveEnable = false;//是否允许移动窗口
        ...
        public VMHWindowControl()
        {
            ...
            WindowH._hWndControl.ShowMessageEvent += HWndCtrl_ShowMessageEvent;
            if (LicenseManager.UsageMode == LicenseUsageMode.Designtime)
            {
                return;
            }
        ...
        {
            if (!crossVisible_strip.Checked) return;
            if (hv_image == null || !hv_image.IsInitialized()) return;

            try
            {
                // 保存当前颜色设置，避免影响其他绘制
                HTuple originalColor = new HTuple();
                //mCtrl_HWindow.HalconWindow.GetColor(out originalColor);
            ...
            {
                Debug.WriteLine($"PaintCross error: {ex.Message}");
            }
        }
        ...
        /// <summary>
        /// 隐藏底部状态栏（显示图像坐标/灰度值）
        /// 当外部提供CAD坐标显示时调用，避免图像坐标与CAD坐标混淆
        /// </summary>
        ...
        public void MoveImageEnabled()
        {
            try
            {
                this.WindowH.MouseMoveImage(isMoveEnable);
            }
            catch (Exception) { }
        }
        /// <summary>
