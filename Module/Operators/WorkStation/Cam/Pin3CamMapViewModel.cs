...
{
    public class Pin3CamMapViewModel : CamHighCmpViewModel<string>
    {
        private string _pinDisplayName = "Pin3飞拍位置";
        public string PinDisplayName
        {
            get => _pinDisplayName;
            set => SetProperty(ref _pinDisplayName, value);
        }
        protected override int CamIndex => 3; // 标识当前处理Pin3
        //private readonly Task2 _task2;
        public Pin3CamMapViewModel(
        ...
        protected override async void MoveToPoint(PointViewModel point)
        {
            ...
            await Task.Run(() =>
            {
                //_task2.GantryStation_MoveToVisionInspPosn3(point.X, point.Y);
            });
        }
    }
}
