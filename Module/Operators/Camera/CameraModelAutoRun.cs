...
namespace Framework.Models
{
    public partial class CameraModel
    {
        ...
        public void SaveToJson()
        {
            ...
            JsonService.ObjectToFile(settings, "./Config/CameraSettings.json");

            WindowAutoClosedSuccess window = new WindowAutoClosedSuccess("保存完成");
            _ = window.ShowDialog();

        }
        public bool LoadFromJson(string json)
        {
            var settings = JsonService.JObjectFromFile<CameraSettings>(json);
            if (settings != null)
            {
                RotateAngle = settings.RotateAngle;
