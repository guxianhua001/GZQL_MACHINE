...
namespace Framework.Models
{
    public class ImagePool : BindableBase
    {
        ...
        private void ExecuteClearPool()
        {
            ...
            {
                DataContext = new ErrorMessageViewModel("szTitle", " 上料时检测到当前卡槽不存在晶圆盘，请确认" +
                            "\r\n 点击 YES 重试 ；\r\n 点击 NO 换提篮下一层；\r\n 点击 Cancel 退出流程", nLevel, "YES", "NO", "Cancel", "关闭"),
                Height = 441,
                Width = 755,
            };
            if (nLevel == 0)
                ...
                win.ShowDialog();
            ErrorMessageViewModel vm = (ErrorMessageViewModel)win.DataContext;
            int res = vm.Result;
        }
